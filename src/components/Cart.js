...
{cart.map(product=>{
    return(
     ...
     <div className="cart-center" key={product.id}>
       <div className="img-cart">
           <img src={product.img} src={`/RS.{product.img}`} alt="img"/>
       </div>
    ...
    <p>Rs. {product.price*product.count}</p>
       </div>
...
<h3><span>amount:</span>Rs.{total}</h3>

<div className="shipping">
<h3><span>shipping: </span>{total>90?'free':`+Rs.{shipping}`}</h3>
</div>
<h3><span>total amount :</span>Rs.{total>90 ?total:total +shipping}</h3>
